...
- The project now includes a **regime-conditioned DRMV variant** that feeds the robust branch with two-state market-factor regime inputs.
...
- The new regime-conditioned allocation inputs are separate from that heuristic tagging block: they use a lightweight two-state market-factor mixture to produce time-varying mean/covariance estimates for the DRMV branch.
